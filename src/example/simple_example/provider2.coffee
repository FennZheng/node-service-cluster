...
ServiceBundles.on(ServiceBundles.EVENT_READY, ()->
	...
	newServer(9190)
	newServer(9191)
	ServiceBundles.register(TEST_SERVICE_REGISTRY, "127.0.0.1:9190", "Meta-ddd", (err, result)->
		console.log("register result:#{result}, err:#{err}")
		ServiceBundles.register(TEST_SERVICE_REGISTRY, "127.0.0.1:9191", "Meta-ddd", (err, result)->
			console.log("register result:#{result}, err:#{err}")
		)
	)
)
